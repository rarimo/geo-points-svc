...
// TODO: fix compile errors, rewrite tests to satisfy the current flow, add new tests for QR-code and poll events
...
func setApiURL() {
	...
	if err != nil {
		panic(fmt.Errorf("failed to figure out listener from service config: %w", err))
	}
...
func initGenesisRef() {
	...
	req := requests.EditReferralsRequest{
		Nullifier: genesisBalance,
		Count:     1,
		Infinity:  true,
	}

	var resp editReferralsResponse
	err = requestWithBody("private/referrals", "POST", "", "", req, &resp)
	if err != nil {
		panic(fmt.Errorf("failed to edit referrals: %w", err))
	}

	genesisCode = resp.Ref
}
...
func TestActivateBalance(t *testing.T) {
	var (
		nullifierShared = nextN()
		otRefCode       string
	)
...
func TestVerifyPassport(t *testing.T) {
	...
	t.Run("VerifyPassport", func(t *testing.T) {
		resp, err := verifyPassport(referee)
		require.NoError(t, err)
...
func TestEventsAutoClaim(t *testing.T) {
	t.Run("PassportScanAutoclaim", func(t *testing.T) {
		...
		respVerifyStatus, err := verifyPassport(n)
		require.NoError(t, err)
	...
	t.Run("ReferralSpecificAutoclaim", func(t *testing.T) {
		...
		respVerifyStatus, err := verifyPassport(n1)
		require.NoError(t, err)
		require.True(t, respVerifyStatus.Data.Attributes.Claimed)
		...
		_, err = verifyPassport(n2)
		require.NoError(t, err)

		respEvents, err := getEvents(n1, models.TypeReferralSpecific)
		require.NoError(t, err)
...
func TestClaimEvent(t *testing.T) {
	t.Run("WithoutPassport", func(t *testing.T) {
		...
		respEvents, err := getEvents(n, models.TypeFreeWeekly)
		require.NoError(t, err)
		require.Equal(t, 1, len(respEvents.Data))
...
func TestLevels(t *testing.T) {
	var (
		...
		evTypePassport = globalCfg.EventTypes().Get(models.TypePassportScan)
		evTypeWeekly   = globalCfg.EventTypes().Get(models.TypeFreeWeekly)
	...
	passportResp, err := verifyPassport(nullifier)
	require.NoError(t, err)
	...
	eventID := getAndValidateSingleEvent(t, nullifier, models.TypePassportScan, status)
	...
	assert.Equal(t, 0, countWithStatus(*refCodes, data.StatusInfinity))

	eventID = getAndValidateSingleEvent(t, nullifier, models.TypeFreeWeekly, data.EventFulfilled)
	...
	assert.Equal(t, 1, countWithStatus(*refCodes, data.StatusInfinity))
}

func countWithStatus(codes []resources.ReferralCode, status string) int {
	count := 0
	for _, v := range codes {
		if v.Status == status {
			count++
...
func TestReferralCodeStatuses(t *testing.T) {
	// infinite code is tested in TestLevels
	...
	t.Run("AwaitingCode", func(t *testing.T) {
		...
		respVerifyStatus, err := verifyPassport(n2)
		require.NoError(t, err)
		require.True(t, respVerifyStatus.Data.Attributes.Claimed)
...
func createBalance(nullifier string) (resp resources.BalanceResponse, err error) {
	body := resources.Key{
		ID:   nullifier,
		Type: resources.CREATE_BALANCE,
	}
...
func verifyPassport(nullifier string) (resp resources.EventClaimingStateResponse, err error) {
	proof := baseProof
	anonID := nullifier[2:]
	body := verifyPassportBody(nullifier, anonID, &proof)
	sig, err := globalCfg.SigCalculator().PassportVerificationSignature(nullifier, anonID)
	if err != nil {
		return
	}

	err = requestWithBody(balancesEndpoint+"/"+nullifier+"/verifypassport", "POST", nullifier, sig, body, &resp)
	return
}
...
func verifyPassportBody(nullifier, anonymousID string, proof *zkptypes.ZKProof) resources.VerifyPassportRequest {
