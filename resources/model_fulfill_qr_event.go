...
// MustFulfillQrEvent - returns FulfillQrEvent from include collection.
// if entry with specified key does not exist - returns nil
// if entry with specified key exists but type or ID mismatches - panics
func (c *Included) MustFulfillQrEvent(key Key) *FulfillQrEvent {
	var fulfillQREvent FulfillQrEvent
	if c.tryFindEntry(key, &fulfillQREvent) {
		return &fulfillQREvent
	}
	return nil
}
